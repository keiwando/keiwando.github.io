...
function colorToPremulStringUI8(col) {
  let a = col[3]
...
function blendErase(src, dst) {
  return dst
}

function blendDarken(src, dst) {
  return Math.min(src, dst)
}

function blendMultiply(src, dst) {
  return src * dst
}

function blendColorBurn(src, dst) {
  let color = 1 - (1 - dst) / src
  return dst == 1 ? 1 : (src == 0 ? 0 : color)
}

function blendLinearBurn(src, dst) {
  return Math.max(src + dst - 1, 0)
}

function blendLighten(src, dst) {
  return Math.max(src, dst)
}

function blendScreen(src, dst) {
  return src + dst - src * dst
}

function blendColorDodge(src, dst) {
  const color = dst / (1 - src)
  return dst == 0 ? 0 : (src == 1 ? 1 : color)
}

function blendAdd(src, dst) {
  return Math.min(1, src + dst)
}

function blendOverlay(src, dst) {
  if (dst <= 0.5) {
    return Math.min(1, 2 * src * dst)
  } else {
    return Math.max(0, 1 - 2 * (1 - src) * (1 - dst))
  }
}

function blendSoftLight(src, dst) {
  if (src <= 0.5) {
    return (2 * src - 1) * (dst - Math.pow(dst, 2)) + dst
  } else {
    return (2 * src - 1) * (Math.pow(dst, 2) - dst) + dst
  }
}

function blendHardLight(src, dst) {
  return blendOverlay(dst, src)
}

function blendVividLight(src, dst) {
  if (src <= 0.5) {
    const color = 1.0 - (1.0 - dst) / (2.0 * src);
    return src == 0.0 ? 0.0 : (dst == 1.0 ? 1.0 : color);
  } else {
    const color = dst / (2.0 * (1.0 - src));
    return src == 1.0 ? 1.0 : (dst == 0.0 ? 0.0 : color);
  }
}

function blendLinearLight(src, dst) {
  return Math.min(1, dst + 2 * src - 1)
}

function blendPinLight(src, dst) {
  if (src <= 0.5) {
    const color = 1.0 - (1.0 - dst) / (2.0 * src);
    return src == 0.0 ? 0.0 : (dst == 1.0 ? 1.0 : color);
  } else {
...
function blendDifference(src, dst) {
  return Math.abs(src - dst)
}

function blendExclusion(src, dst) {
  return Math.min(1, src + dst - 2 * src * dst)
}

function blendSubtract(src, dst) {
  return Math.max(0, dst - src)
}

function blendDivide(src, dst) {
  return dst == 0.0 ? 0.0 : (src == 0.0 ? 1.0 : dst / src);
}


function blendFuncForBlendMode(blendMode) {

  switch (blendModeName(blendMode)) {
